...
\begin{defn}
  ...
  The $\post*$ operator can be defined as a repeated application of $\post$ along a given path. More formally, it is $\post* : \heappats \times V \times V \to \heappats$, where $V$ is the set of vertices in the unwinding. This is only applicable to pairs of vertices that have a direct ancestor-successor relationship.
\end{defn}
...
\begin{algorithm}[ht]
  ...
  \caption{$\newcandidate$: takes as input a program location $l_i$, map $\psi$, and unfolding $\mathcal{U}(\pi)$ of path $\pi$, and interacts with the Oracle $\mathcal{O}$ to find a new candidate for $l_i$ using a set of positive ($H_i^{+}$) and negative ($H_i^{-}$) examples.}

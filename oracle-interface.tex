...
Consider a program that constructs a singly-linked list where the $\true$ value is always reachable from the head of the list.

\subsubsection{Inverted Tree}
An inver

\subsubsection{Two-part List}
A list that consists of two segments. For a fixed predicate $p$ the first few nodes are all $\true$, and the rest are all $\false$, followed by \nilconst.
